...
`, ''],
  ...
  ['a place counts as inside a city whose eastern edge it is past', String.raw`place.lng <= c.lngMax`, String.raw`place.lng <= c.lngMax + 10`],
  ['the app claims to cover everywhere when it has not been told which cities it covers', String.raw`const inServiceArea = (p, cities) => (cities?.length ? !!cityAt(p, cities) : validPlace(p));`, String.raw`const inServiceArea = (p, cities) => validPlace(p);`],
