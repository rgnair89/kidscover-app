...
await ui.click('auth-switch'); await ui.type('first-name', 'Dee'); await ui.type('last-name', 'Rao'); await ui.type('email', 'dee@x.in'); await ui.type('password', 'longenough1'); await ui.click('signup-gender-man'); await ui.click('auth-submit');
...
st = seed(); ui = await mount(st);
await signIn(ui, 'ann@x.in', 'password1');
check('the button at the top right is a drawn parent, there from the first moment', await waitFor(() => !!ui.id('top-avatar')) && ui.id('top-avatar').tagName.toLowerCase() === 'svg');
check('...with a small mark on it while the profile is unfinished', !!ui.id('profile-dot'));
check('the list says the profile is unfinished, with a button that goes straight there', !!ui.id('profile-nudge') && !!ui.id('profile-finish'), ui.text().slice(0, 200));
await ui.click('profile-finish');
check('the profile opens with the names, the six pictures and the four answers', await waitFor(() => !!ui.id('profile-card')) && !!ui.id('profile-first') && !!ui.id('profile-last')
  && ui.all('avatar-parent_').length === 6 && ['woman', 'man', 'other', 'prefer_not_to_say'].every((g) => !!ui.id('profile-gender-' + g)));
check('...and a welcome, because there is something left to fill in', !!ui.id('profile-welcome'));
await ui.click('profile-save');
check('saving before answering asks first, and writes nothing to the database', await waitFor(() => !!ui.id('profile-problem')) && (st.profiles.u1 ?? {}).gender === undefined, JSON.stringify(st.profiles));
await ui.click('profile-gender-prefer_not_to_say');
await ui.click('avatar-parent_six');
await ui.click('profile-save');
check('...and once answered it is saved, the answer and the chosen picture together', await waitFor(() => !!ui.id('profile-done')) && (st.profiles.u1 ?? {}).gender === 'prefer_not_to_say' && (st.profiles.u1 ?? {}).avatar === 'parent_six', JSON.stringify(st.profiles));
check('...and nothing a parent may not change was sent along', Object.keys(st.profiles.u1 ?? {}).every((k) => ['first_name', 'last_name', 'gender', 'avatar', 'language', 'notify_push'].includes(k)), Object.keys(st.profiles.u1 ?? {}).join(','));
check('the mark on the button goes away as soon as it is saved, without signing in again', !ui.id('profile-dot') && !ui.id('profile-welcome'));
await ui.click('top-back');
check('...and the reminder on the list is gone too', await waitFor(() => !!ui.id('search')) && !ui.id('profile-nudge'));
await ui.unmount();
st = seed(); st.profiles = { u1: { first_name: 'Bo', last_name: 'Sen', gender: 'man', avatar: 'auto' } }; ui = await mount(st);
await signIn(ui, 'ann@x.in', 'password1');
check('someone who finished their profile earlier is not nagged about it', await waitFor(() => !!ui.id('top-avatar')) && !ui.id('profile-dot') && !ui.id('profile-nudge'));
await ui.unmount();
st = seed(); ui = await mount(st);
await waitFor(() => !!ui.id('auth-submit'));
await ui.click('auth-switch');
await waitFor(() => !!ui.id('first-name'));
check('joining asks how a parent would like to be described, with the same four answers', ['woman', 'man', 'other', 'prefer_not_to_say'].every((g) => !!ui.id('signup-gender-' + g)));
await ui.type('first-name', 'Dee'); await ui.type('last-name', 'Rao');
await ui.type('email', 'dee@x.in'); await ui.type('password', 'password9');
await ui.click('auth-submit');
check('...and will not create the account until it is answered', await waitFor(() => !!ui.id('auth-error')) && !st.authCalls.some(([kind]) => kind === 'signup'), ui.id('auth-error')?.textContent);
await ui.click('signup-gender-woman');
await ui.click('auth-submit');
